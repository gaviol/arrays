...
function procesarPedido(pedido) {
    const cliente = pedido.shift(); // Eliminar el primer elemento (cliente)
    pedido.unshift('bebida'); // Añadir 'bebida' al inicio
    pedido.push(cliente); // Añadir el cliente al final
    return pedido;
}
...
function buscaPalabras(words, palabraBuscada) {
     const index = words.indexOf(palabraBuscada);
    if (index === -1) return []; 
    return words.slice(index + 1);
}
const words = ['hola', 'mundo', 'javascript', 'es', 'genial'];
const palabraBuscada = 'javascript';
console.log(buscaPalabras(words, palabraBuscada));
...
function findJavaScript(matrix) {
    for (let i = 0; i < matrix.length; i++) {
        for (let j = 0; j < matrix[i].length; j++) { // Iterar a través de cada fila
            if (matrix[i][j] === 'JavaScript') { // Si encuentra 'JavaScript'
